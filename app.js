...
function renderRoadmap() {
  ...
  const steps = [
    ...
    { label: "3단계", title: "Proud 진입 & 최종 궁극 무기 획득", desc: "최종 프라우드 난이도를 뚫고 '바하무트의 분노' 프로토 바하무트 퀘스트 뺑뺑이 루프를 돕니다. 최강인 터미너스(궁극) 무기를 획득하고 대미지 상한 65 진 세팅을 갖춥니다." },
...
function renderChecklist() {
  ...
  const items = [
    ...
    { id: "dmg_cap65", label: "대미지 상한 진 V 65레벨 풀셋팅 완성" },
...
function openModal(charId) {
  ...
  document.getElementById("modal-playtip").innerHTML = `
    <strong>💡 메인 컨트롤 팁:</strong> ${char.play_tip || "평타 공격 연타 후 특수 피니시 콤보를 꽂아 상시 대미지 상한선을 뽑아내는 안정적인 사냥 방식을 장착하십시오."}
  `;
...
function renderModalPhase() {
  ...
  if (char.id === "fraux") {
    const frauxPhases = [
      {
        ...
        sigils: [
          ...
          { name: "공격력 V / IV", order: "2순위", rating: "9/10", sub: "깡공 증가 진", desc: "초반 대미지 상한이 낮으므로 깡공격을 올려 잡몹전과 보스전의 클리어 타임을 단축시킵니다." },
        ...
        desc: "메인 시나리오를 밀고 있는 성장기입니다. 대미지 상한이나 전용 진의 셋팅을 전혀 신경 쓸 필요가 없으며, 체력(수호)과 공격력을 골고루 섞어 스토리 보스를 쾌적하게 클리어하는 것을 최우선 목표로 잡습니다."
      },
      {
        weapons: [
          ...
          { name: "데블 핑거 [수호 무기]", order: "2순위", rating: "6/10 (생존용)", desc: "극초반 보스의 패턴 대미지를 버틸 피통이 부족할 때 대장간에서 즉시 임시 거치용으로 제작하여 착용합니다." }
        ],
        ...
        sigils: [
          ...
          { name: "대미지 상한 V", order: "2순위", rating: "9/10", sub: "깡공 혼신/폭군 진", desc: "이 구간에서는 상한 진은 1~2개(30레벨)만 가볍게 넣고, 깡공 혼신 진을 섞어 상한 딜에 안정적으로 도달하게 합니다." },
        ...
        desc: "스토리 엔딩을 막 보신 상태입니다. 이 시기에는 대미지 상한선이 낮으므로 무리하게 상한 진을 도배하기보다는 스팅어 무기를 강화하여 크리티컬 확률 100%를 달성하고, 깡공(혼신/폭군) 진을 1~2개 섞어 대미지를 상한선에 안착시키는 것을 목표로 삼습니다."
      },
      {
        weapons: [
          ...
          { name: "베가르드 권갑 [익스큐셔너 무기/스턴]", order: "2순위", rating: "7/10 (기졸용)", desc: "보스의 그로기를 빠르게 유도해 아군의 링크 어택 시동을 돕는 유용한 유틸 무기." }
        ],
        ...
        sigils: [
          ...
          { name: "대미지 상한 V", order: "2순위", rating: "10/10", sub: "추가 대미지 V", desc: "3~4개 기용하여 대미지 상한을 45~50레벨 수준까지 끌어올립니다." },
      ...
      },
      {
        ...
        sigils: [
          ...
          { name: "대미지 상한 V", order: "2순위", rating: "10/10", sub: "공격력/상한 한돌", desc: "65레벨 풀셋 완성. 무기 스킬 강화로 제한을 완벽하게 풉니다." },
          ...
          { name: "근성 V (Aegis)", order: "6순위", rating: "8/10", sub: "수호 V / 방어 V", desc: "사망 대미지를 입어도 피 1을 남겨 무적 상태로 생존하게 하는 생존 시길." }
        ],
        focus: "Proud 최종 바하무트 토벌 및 루시퍼 진입 준비 기간입니다. 은천의 빛을 녹여 각성 무기 10단계 각성 완수를 최우선으로 진행합니다.",
        desc: "프라우드 난이도를 뚫고 프로토 바하무트에서 궁극 무기를 파밍하는 시기입니다. 대미지 상한을 65레벨 풀로 채워야 하며, 모든 속성 공격을 약점으로 찌르게 해주는 유리 속성 변환 V를 획득하여 장착하는 것이 핵심입니다."
      },
      {
        weapons: [
        ...
        sigils: [
          ...
          { name: "종말의 진 베타", order: "2순위", rating: "10/10", sub: "종말의 진 감마", desc: "어빌리티 자체 피해 배율을 늘려 파워 플랜트 한 방 대미지를 극한으로 증강시킵니다." },
        ...
        desc: "라그나로크 확장팩의 최종 루시퍼/벨제붑을 격파한 종결 상태입니다. 루시퍼가 드롭하는 종말의 진 2개를 채용해 추가 대미지 상한 30%를 더 뚫어주며, 200레벨 초월 무기의 압도적인 깡공을 기반으로 최종 스펙을 완성합니다."
      }
    ];
    ...
    const cagliostroPhases = [
      {
        weapons: [
          ...
          { name: "우로보로스 [Ruins/할로우 무기]", order: "2순위", rating: "8/10", desc: "무난한 기본 스펙을 올려주는 초반 징검다리 스태프." }
        ],
        skills: "판타스마고리아 (공방크리 버프) 및 힐 스킬 채용. 스토리 진행 중 아군의 회복약 소모를 줄이고 생존력을 크게 올립니다.",
        sigils: [
          ...
          { name: "공격력 V / IV", order: "3순위", rating: "8/10", sub: "폭군 V / 결사 V", desc: "TGV 등 기본 공격 스킬 딜 보강용." }
        ],
        focus: "메인 스토리 스토리 진행 중. 판타스마고리아 전체 공방 버프 버스트 타이밍 익히기.",
        desc: "스토리 퀘스트 중 칼리오스트로의 역할은 든든한 아군 버퍼이자 서브 힐러입니다. 판타스마고리아를 쿨마다 돌리고, 아군이 위험할 때 힐을 뿌려주는 기본 생존과 버프 순환을 손에 익히는 구간입니다."
      },
      {
        weapons: [
          { name: "베가르드 사수 [스팅어 무기]", order: "1순위", rating: "10/10 (필수)", desc: "초반 크리 확률 100% 세팅을 빠르게 구축하여, 아군 버프 판타스마고리아 가동 시 딜 기여 크리 발생률을 보정하는 징검다리 스태프." },
          { name: "우로보로스 [Ruins/할로우 무기]", order: "2순위", rating: "6/10 (대체용)", desc: "스팅어 제작 재료가 모자랄 때 대장간에서 임시 징검다리로 거쳐갈 때 유용한 스태프." }
        ],
        skills: "판타스마고리아 (공방크리 30% 증가 버프) & TGV (광선 쿨감기) 필수 배치. 아군의 딜링 상한 안착을 돕는 보조 딜러 세팅.",
        sigils: [
          { name: "크리티컬 확률 V", order: "1순위", rating: "10/10", sub: "혼신 V / 폭군 V", desc: "스팅어 창과 연동해 100% 크리를 맞춰주고, 깡공 진을 섞어 판타스마고리아가 켜졌을 때 상한 딜이 바로 나오게 보정합니다." },
          { name: "대미지 상한 V", order: "2순위", rating: "9/10", sub: "공격력 V", desc: "상한 1~2개(30레벨)만 가볍게 세팅하여, 초반 부족한 상한치를 미세 조정하는 데 집중합니다." },
          { name: "혼신 V", order: "3순위", rating: "8/10", sub: "폭군 V", desc: "버프와 맞물려 기본 대미지를 상한선 부근까지 도달시켜 주는 우수한 딜 진." },
      ...
      },
      {
        ...
        skills: "판타스마고리아 & TGV & 알렉산드리아 (스턴 게이지 시동용) & 리조마타 (원거리 즉발 소생 필수 채용).",
        sigils: [
          { name: "극치의 진리", order: "1순위", rating: "10/10", sub: "대장간 전용 진 합성", desc: "피니시 적중 시 쿨타임 감소가 걸려 판타스마고리아 공방크리 버프를 100% 상시 무한 유지하게 만드는 원동력." },
          { name: "대미지 상한 V", order: "2순위", rating: "10/10", sub: "일반 딜진", desc: "대미지 상한 레벨을 45~50레벨로 맞추어 TGV의 타격당 대미지를 극대화." },
      ...
      },
      {
        ...
        skills: "판타스마고리아 & TGV & 알렉산드리아 & 리조마타 구성. 판타스마고리아를 계속 켜며 적 스턴치가 쌓였을 때 오의 체인을 시작합니다.",
        sigils: [
          { name: "유리 속성 변환", order: "1순위", rating: "10/10", sub: "대체 불가", desc: "서포터도 준수한 딜을 넣게 만들어주는 개사기 진. TGV 광선과 조화가 매우 훌륭합니다." },
          { name: "대미지 상한 V", order: "2순위", rating: "10/10", sub: "퀵 어빌리티 V", desc: "65레벨 풀셋팅 완성. 퀵 어빌리티 진을 추가로 섞어 전용 진과 쿨감 역시너지를 극대화합니다." },
      ...
      },
      {
        ...
        skills: "판타스마고리아 & TGV & 알렉산드리아 & 리조마타 고정. 아군 부활 및 상시 공방 30% 증가 버프로 전장을 안정적으로 리드합니다.",
        sigils: [
          ...
          { name: "추가 대미지 V+", order: "3순위", rating: "10/10", sub: "추가 대미지 V", desc: "유틸리티(포션 수/회피성능/고양)가 2옵션으로 붙은 최종 종결 진 세트." },
          { name: "유리 속성 변환", order: "4순위", rating: "10/10", sub: "대체 불가", desc: "필수 장착." },
          { name: "퀵 어빌리티 V", order: "5순위", rating: "9/10", sub: "맹렬 V", desc: "전용 진과 융합하여 판타스마고리아의 쿨타임을 15초 미만으로 단축시킵니다." },
    ...
    const zetaPhases = [
      {
        weapons: [
          ...
          { name: "황금의 삼지창 [익스큐셔너 무기/스턴]", order: "2순위", rating: "8/10", desc: "보스의 기절치를 보충하여 초반 링크 어택 기회를 확대시키는 보조 창." }
        ],
        skills: "인피니트 원더스 (알베스 표식 부여용) & 알베스의 창 (공중 돌진기) 필수 배치. 표식 디버프 ➡️ 도약 ➡️ 공중 콤보 연계를 연습하는 단계.",
        sigils: [
      ...
      },
      {
        weapons: [
          ...
          { name: "황금의 삼지창 [익스큐셔너 무기/스턴]", order: "2순위", rating: "6/10 (대안)", desc: "스팅어 무기 제작 전까지 적 보스를 기절 그로기로 눕히기 위한 유용한 창." }
        ],
        skills: "인피니트 원더스 (알베스 표식 부여용) & 알베스의 창 (공중 돌진기) 필수 배치. 표식 디버프 ➡️ 도약 ➡️ 공중 콤보 연계를 연습하는 단계.",
        sigils: [
          { name: "크리티컬 확률 V", order: "1순위", rating: "10/10", sub: "혼신 V / 폭군 V", desc: "스팅어 창과 융합하여 체공 콤보 크리 발생률을 보장하고 깡공 진을 섞어 대미지를 상한치로 밀어 올립니다." },
          { name: "대미지 상한 V", order: "2순위", rating: "9/10", sub: "스턴 V 1개", desc: "상한 1~2개(30레벨)만 가볍게 채용하며 공중 루프 도중 보스의 그로기 타격을 유도하기 위한 스턴치를 우선 확보합니다." },
      ...
      },
      {
        weapons: [
          ...
          { name: "투기용 창 [할로우 무기]", order: "2순위", rating: "7/10 (대체)", desc: "각성 재료 은천의 빛 파밍이 고달플 때 대장간에서 높은 수치로 거쳐갈 수 있는 무기." }
        ],
        skills: "인피니트 원더스 & 알베스의 창 & 렐름 마제스티 (적 광역 패턴 튕겨내기 반격기) & 시그노 드라이브 (공격력 증가).",
        sigils: [
          ...
          { name: "대미지 상한 V", order: "2순위", rating: "10/10", sub: "연계 공격 V", desc: "상한 3~4개 장착하여 45~50레벨 상한을 확보, 콩콩이 딜 누수를 방지합니다." },
      ...
      },
      {
        ...
        skills: "인피니트 원더스 & 알베스의 창 & 렐름 마제스티 & 시그노 드라이브 구성. 보스가 날리는 광역기를 카운터 렐름스로 씹으며 맞공격을 날립니다.",
        sigils: [
          { name: "유리 속성 변환", order: "1순위", rating: "10/10", sub: "대체 불가", desc: "상성 무관 약점 딜 보정. 추가 대미지 V 3개 조합과 결합하여 공중 콩콩이 참격 한 발 한 발을 우주급으로 강화시킵니다." },
          { name: "대미지 상한 V", order: "2순위", rating: "10/10", sub: "한계돌파 크리 20%", desc: "65레벨 풀셋팅 필수 도달. 한돌로 크리를 당겨와 남은 진 슬롯에 배수/포션/자동부활을 든든하게 다집니다." },
          { name: "추가 대미지 V", order: "3순위", rating: "9/10", sub: "결사 V", desc: "저스트 평강공 한 타 한 타마다 20%의 추가 딜을 폭발시킵니다." },
      ...
      {
        weapons: [
          { name: "게이볼그 [초월 200레벨]", order: "1순위", rating: "10+/10 (종결)", desc: "DLC 라그나로크 최종 무기 초월 200레벨 달성. 공중 에어 콤보의 물리 타격 대미지를 무지막지하게 보정하며 최종 옵션 슬롯을 확보합니다." },
          { name: "천상창 [DLC 제작 무기]", order: "2순위", rating: "9/10 (대안)", desc: "얼티메이트 메모리를 활용해 제작하는 고스탯의 DLC 특화 창." }
        ],
        skills: "인피니트 원더스 & 알베스의 창 & 렐름 마제스티 & 시그노 드라이브 고정. 루시퍼의 공중 패턴을 함께 날아오르며 요격합니다.",
        sigils: [
    ...
    const beatrixPhases = [
      {
        weapons: [
          { name: "엠브라스크의 검 [스팅어 무기]", order: "1순위", rating: "9/10", desc: "저스트 타이밍 피니시의 치명타 확률을 올려 초반 교전 속도를 크게 단축시키는 크리티컬 특화 대검." },
          { name: "다크 액스 [익스큐셔너 무기/스턴]", order: "2순위", rating: "8/10", desc: "제작이 쉽고 무난하게 스펙을 깡으로 올려주는 초반 징검다리 무기." }
        ],
        skills: "언체인드 (자가 무적기) 필수 채용. 피를 깎아 맞딜을 하는 베아트리스의 급사 리스크를 최우선 차단합니다.",
        sigils: [
        ...
        desc: "체력 소모와 저스트 입력을 동시에 챙겨야 하는 중급자용 캐릭터입니다. 스토리 구간에서는 피통이 늘 요동치므로, 위험할 때 언체인드 무적을 즉시 켜고 평타 저스트 타이밍에 맞춰 ㅅ 버튼을 꽂아 인과 탐식 콤보를 연습합니다."
      },
      {
        weapons: [
          { name: "엠브라스크의 검 [스팅어 무기]", order: "1순위", rating: "10/10 (필수)", desc: "저스트 공격 시 치명타 발생률을 확보하기 위한 크리티컬 특화 징검다리 대검. 초반 마스터리 해금 전 필수 장비." },
          { name: "다크 액스 [익스큐셔너 무기/스턴]", order: "2순위", rating: "6/10 (대체용)", desc: "스팅어 대검이 만들어지기 전 거치용 대장간 검." }
        ],
        skills: "하츠 온 파이어 (마검 게이지 즉시 충전 난무) & 언체인드 (자가 무적기) 배치. 자가 체력 소모를 하츠 온 파이어 충전 후 강공 저스트 공격으로 빠르게 극복하는 연습기.",
        sigils: [
          ...
          { name: "대미지 상한 V", order: "2순위", rating: "9/10", sub: "체력 V", desc: "상한 진 1개만 임시 기용. 초반 낮은 피통 리스크를 보완하기 위한 가성비 위주 세팅." },
          { name: "배수 V (Enmity)", order: "3순위", rating: "8/10", sub: "결사 V", desc: "피가 자주 깎이는 베아트리스 전용 최고 등급 딜 보조진." },
          { name: "포션 보유 수 V", order: "4순위", rating: "9/10", sub: "흡혈 V", desc: "인과 탐식 폭딜 도중 소모되는 체력을 즉시 100% 복구할 코어 포션 진." },
        ...
        focus: "초반 저스트 타이밍 습득 단계. 하츠 온 파이어로 게이지를 채운 뒤, 어빌 가동 후 정확한 저스트 ㅅ 공격 손맛을 습득하는 구간입니다.",
        desc: "스토리 엔딩을 막 보신 상태입니다. 이 시기에는 대미지 상한선이 낮으므로 무리하게 상한 진을 도배하기보다는 스팅어 무기를 강화하여 크리티컬 확률 100%를 달성하고, 깡공(혼신/폭군) 진을 1~2개 섞어 대미지를 상한선에 안착시키는 것을 목표로 삼습니다."
      },
      {
        weapons: [
          { name: "에라스티아 [각성 무기/어센션]", order: "1순위", rating: "9/10 (주력)", desc: "150레벨 풀강 및 대장간 10단계 풀각성 달성. 인과 탐식 폭딜 수치와 함께 자가 피소모 리스크를 견딜 든든한 깡체력을 대폭 늘려주는 주력 검." },
          { name: "불멸의 수호검 [수호 무기/체력]", order: "2순위", rating: "6/10 (방어용)", desc: "각성 무기가 개방되기 전 피통 위주의 안정적 성장을 돕는 방어용 무기." }
        ],
        skills: "하츠 온 파이어 & 언체인드 & 라이딩 프리 (이동 및 회피 무적 시간 버프) & 썬더버드 (방깎 광역 폭발).",
        sigils: [
          ...
          { name: "대미지 상한 V", order: "2순위", rating: "10/10", sub: "연계 공격 V", desc: "3~4개 기용하여 대미지 상한 레벨 45를 구축, 마검 참격의 최대 피해량을 해방." },
      ...
      {
        weapons: [
          { name: "플로렌시아 [궁극 무기/터미너스]", order: "1순위", rating: "10/10 (최종)", desc: "프로토 바하무트 드롭 최종 대검. 베아트리스의 인과 탐식 상태 저스트 참격 딜링을 리링크 한계 대미지 상한선까지 수직 상승시킵니다." },
          { name: "에라스티아 [각성 완수]", order: "2순위", rating: "8/10 (대안)", desc: "최종 무기를 제작하고 파밍하는 단계에서 보스 카운터 딜을 충분히 메워줄 풀각성 장비." }
        ],
        skills: "하츠 온 파이어 & 언체인드 & 라이딩 프리 & 썬더버드 구성. 적의 맹공 기믹 때 언체인드 무적을 키고 맞공격 대칭 딜을 전개합니다.",
        sigils: [
          ...
          { name: "대미지 상한 V", order: "2순위", rating: "10/10", sub: "한계돌파 크리 20%", desc: "65레벨 풀셋팅 도달. 한돌로 크리를 당겨와 남은 진 슬롯에 배수/포션/자동부활을 든든하게 다집니다." },
          { name: "추가 대미지 V", order: "3순위", rating: "9/10", sub: "결사 V", desc: "저스트 평강공 한 타 한 타마다 20%의 추가 딜을 폭발시킵니다." },
      ...
      {
        weapons: [
          { name: "플로렌시아 [초월 200레벨]", order: "1순위", rating: "10+/10 (종결)", desc: "DLC 라그나로크 최종 무기 초월 200레벨 해금. 인과 탐식 저스트 공격력의 물리 캡 수치를 초월 급으로 상향 조정하며 최종 합성 슬롯을 제공." },
          { name: "칠흑의 무뢰검 [DLC 제작 대검]", order: "2순위", rating: "9/10 (대안)", desc: "루시퍼의 얼티메이트 메모리 재료로 수동 제작하는 확장팩 최적화 속성 대검." }
        ],
        skills: "하츠 온 파이어 & 언체인드 & 라이딩 프리 & 썬더버드 고정. 언체인드 무적의 딜증가 버프를 연계해 보스를 한순간에 녹여냅니다.",
        sigils: [
          ...
          { name: "추가 대미지 V+", order: "3순위", rating: "10/10", sub: "추가 대미지 V", desc: "유틸리티(포션 수/회피성능/고양)가 2옵션으로 붙은 최종 종결 진 세트." },
          { name: "유리 속성 변환", order: "4순위", rating: "10/10", sub: "대체 불가", desc: "필수 장착." },
          { name: "명경지수 V+", order: "5순위", rating: "9/10", sub: "퀵 어빌리티 V / 맹렬 V", desc: "저스트 회피 시 무적과 쿨감 가속을 극대화하여, 하츠 온 파이어 및 언체인드 무적 쿨을 빠르게 돌려 생존과 버스트 루프를 상시 유지하는 스펙." },
    ...
    const defaultPhases = [
      ...
      },
      {
        ...
        sigils: [
          ...
          { name: "대미지 상한 V", order: "2순위", rating: "9/10", sub: "공격력 V", desc: "상한 1~2개(30레벨)만 가볍게 세팅하여, 초반 부족한 상한치를 미세 조정하는 데 집중." },
          { name: "혼신 V", order: "3순위", rating: "8/10", sub: "공격력 증가 V", desc: "깡공 수치를 늘려 대미지 상한에 닿게 만드는 필수 딜진." },
      ...
      },
      {
        ...
        sigils: [
          ...
          { name: "대미지 상한 V", order: "2순위", rating: "10/10", sub: "딜 보정 가호", desc: "대미지 상한 45레벨 수준을 목표로 차분히 강화 시길을 추가해 갑니다." },
      ...
      },
      {
        ...
        sigils: [
          ...
          { name: "대미지 상한 V", order: "2순위", rating: "10/10", sub: "한계돌파 크리 20%", desc: "65레벨 풀셋팅 완료. 깡딜 상한을 모두 확보합니다." },
      ...
      },
      {
        ...
        sigils: [
          ...
          { name: "종말의 진 보조 코드", order: "2순위", rating: "10/10", sub: "일반 합성진", desc: "게이지 충전 혹은 오의 후 대미지 상한을 추가 뚫어주는 확장팩 핵심 시길." },
          { name: "추가 대미지 V+", order: "3순위", rating: "10/10", sub: "추가 대미지 V", desc: "유틸리티(포션 수/회피성능/고양)가 2옵션으로 붙은 최종 종결 진 세트." },
  ...
  const comboGuides = {
    ...
    cagliostro: [
      ...
      { name: "판타스마고리아 극딜 콤보", seq: "판타스마고리아 (L1 + □) ➡️ 알렉산드리아 (L1 + △) ➡️ TGV (L1 + ○ 차징 유지)", desc: "공방 버프를 자신에게 얹은 후 TGV 차징 레이저를 다단 히트시켜 대미지를 누적시킵니다." },
    ...
    zeta: [
      { name: "알베스 루프 체공 콤보 (콩콩이)", seq: "인피니트 원더스 (L1 + □) ➡️ 알베스의 창 (L1 + △) ➡️ (체공 후) □ ➡️ □ ➡️ □ ➡️ 타이밍 맞춰 △ (루프)", desc: "공중 도약 후 표식이 찍힌 적에게 돌진하여 루프 참격을 가하는 제타의 알파이자 오메가인 체공 콤보입니다." },
      { name: "렐름스 카운터 지상 카운터", seq: "렐름 마제스티 (L1 + ○) (패링 성공 시) ➡️ △ (공중 도약) ➡️ □ ➡️ □ ➡️ □ ➡️ △", desc: "보스의 광역기나 포효를 패링 무적으로 맞받아친 다음 즉시 공중 콩콩이 루프로 전환하는 카운터 콤보입니다." },
    ...
    beatrix: [
      { name: "저스트 타이밍 평강 콤보", seq: "하츠 온 파이어 (L1 + □) ➡️ □ ➡️ △ (저스트 입력) ➡️ □ ➡️ △ (저스트 입력)", desc: "강공격 입력 시 정확한 불빛 타이밍에 맞춰 버튼을 누름으로써 공속과 딜을 폭증시키는 콤보입니다." },
      { name: "불사신 무적 폭딜 콤보", seq: "언체인드 (L1 + △) (무적 가동) ➡️ 라이딩 프리 (L1 + ○) ➡️ □ ➡️ △ (저스트) 연속 스팸", desc: "체력이 소모되는 마검 난사 상태를 언체인드 무적으로 버티며 딜을 밀어 넣는 보스 극딜용 콤보입니다." },
      { name: "썬더버드 디버프 연계", seq: "썬더버드 (L1 + ✕) (방깎) ➡️ 하츠 온 파이어 (L1 + □) ➡️ □ ➡️ △ (저스트)", desc: "방어력 감소 디버프를 광역 폭발로 가한 뒤 곧바로 저스트 연타를 꽂는 정석 연계입니다." },
      { name: "SBA 오의 폭발 피니시", seq: "언체인드 발동 ➡️ 저스트 콤보 ➡️ ○ + ✕ (오의)", desc: "무적 버프 상태에서 극딜 후 마지막 오의 피니시를 터트리는 안정적인 종결 콤보입니다." }
    ]
  };
  ...
  const aiRecommendations = {
    ...
    zeta: [
      ...
      { rank: "8위", name: "페르시발", rating: "7.6/10", role: "딜포터", reason: "적 방깎 디버프 순환으로 제타의 콩콩이 피니시 및 에어 어빌리티 대미지 배율을 배로 올려줍니다." },
      { rank: "9위", name: "프라우", rating: "7.3/10", role: "스탠스 딜러", reason: "지상 스탠스 딜을 흔들림 없이 가동하여 제타의 체공 딜링과 훌륭한 더블 딜 시너지를 완성합니다." },
      { rank: "10위", name: "베아트리스", rating: "7.3/10", role: "배수 딜러", reason: "제타의 빠른 오의 가속 타이밍에 맞춰 지상에서 폭발적인 대미지 지원으로 고속 토벌을 유도합니다." }
    ],
    beatrix: [
      ...
      { rank: "8위", name: "페르시발", rating: "7.5/10", role: "딜포터", reason: "공버프와 적 방깎으로 베아트리스가 언체인드 무적 상태에서 극한의 상한 딜을 투사할 발판을 만듭니다." },
  ...
  const defaultAiCompanions = [
    ...
    { rank: "8위", name: "페르시발", rating: "7.6/10", role: "딜포터", reason: "아군 전체 공버프와 적 방어력 깎기를 동시 순환해 파티 전체의 대미지 폭발력에 기여합니다." },
    { rank: "9위", name: "그랑", rating: "7.2/10", role: "올라운더", reason: "힐과 디스펠 정화를 통한 보스 상태이상 억제 및 파티 유틸리티 보강을 매끄럽게 책임집니다." },
    { rank: "10위", name: "지타", rating: "7.2/10", role: "올라운더", reason: "그랑과 비슷하게 힐, 대미지 컷 배리어 및 링크 게이지 수급에 특화되어 어느 파티나 무난하게 투입 가능합니다." }
  ];
...
function getWeaponDetails(charId, genericName) {
  ...
  if (lower.includes("스팅어")) {
    ...
    return { name: charDb.stun, sub: "익스큐셔너 무기 (스턴 특화)" };
